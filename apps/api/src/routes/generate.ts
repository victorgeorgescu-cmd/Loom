...
const FOOTER_PLACEHOLDER = "<CAN-SPAM physical address>";
...
export async function generateRoutes(app: FastifyInstance): Promise<void> {
  app.post("/api/generate", async (req, reply) => {
    ...
    const { templateId, brand: brandSlug, brief } = parsed.data;

    const brand = loadBrand(brandSlug);
    const template = loadTemplate(templateId);

    // 1. Copy from Claude (validated as GeneratedCopy inside).
    ...
    const url = await uploadImage(`${brand.slug}/${project.id}/hero.${ext}`, image.bytes, image.contentType);

    await prisma.projectAsset.create({
      data: { projectId: project.id, kind: "hero", url, prompt: copy.hero_image_brief, bytes: image.bytes.length },
    });
